...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (recvfrom(sockfd, peticion, TAM_MSG - 1, 0,
                     (struct sockaddr *)&broker, &tam_broker) == -1) {
            perror("Error recvfrom"); continue;
        }

        printf("[GestorClaves] Peticion de %s:%d -> \"%s\"\n",
        ...
        if (sendto(sockfd, respuesta, strlen(respuesta), 0,
                   (struct sockaddr *)&broker, sizeof(struct sockaddr)) == -1) {
            perror("Error sendto");
        }
    }
